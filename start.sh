...
# Включаем «режим тишины» для всей системы (глушим системный спам Vast.ai)
exec 3>&2          # Сохраняем оригинальный поток ошибок в дескриптор 3
exec 2>/dev/null   # Направляем текущий поток ошибок в пустоту
...
log_step "05" "ЗАПУСК"
echo -e "${GREEN}✨ Все системы готовы. Приятной работы!${NC}"
echo -e "${GRAY}------------------------------------------------------------${NC}"

# ВОЗВРАЩАЕМ поток ошибок обратно перед запуском сервера
exec 2>&3
