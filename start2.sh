...
set -e
export TERM=xterm
# Заставляем Git и SSH молчать на системном уровне
...
# МЕНЯЕМ ЗДЕСЬ: Новый тестовый репозиторий
...
status_msg "Обновление зависимостей"
pip install --upgrade pip -q
pip install -r requirements.txt -q
status_ok

log_step "03" "НАСТРОЙКА РАСШИРЕНИЙ (ТЕСТ)"
nodes_dir="${COMFYUI_DIR}/custom_nodes"
temp_dep="/tmp/dep_test_repo"

status_msg "Синхронизация ТЕСТОВОГО репозитория"
rm -rf "$temp_dep"
git clone --depth 1 "$DEP_NODES_REPO" "$temp_dep" -q

# Копируем содержимое тестового репо в custom_nodes
# Если в репозитории лежат папки с нодами — они скопируются корректно
cp -r "$temp_dep"/* "$nodes_dir/" || true
# Чистим мусор, если он попал из корня репо
rm -f "$nodes_dir/README.md" "$nodes_dir/LICENSE" "$nodes_dir/.git"
...
log_step "05" "ЗАПУСК ТЕСТОВОЙ СРЕДЫ"
echo -e "${GREEN}✨ Тестовый репозиторий развернут. Поехали!${NC}"
echo -e "${GRAY}------------------------------------------------------------${NC}"
